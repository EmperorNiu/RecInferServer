...
func (this *handle) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Path == "/DictService/seek" && r.Method == "POST" {
		...
		return
	} else if r.URL.Path == "/seek_lru" && r.Method == "POST" {
		embedRequestWithLRU(w, r)
		return
	}
}

func embedRequestWithLRU(w http.ResponseWriter, r *http.Request) {
	...
	if !perfect {
		...
		lru_result := make(map[int][]float32)
		key_len := len(keys)
		...
		if pattern == "cube" {
			...
			for i := range keys {
				if partition[keys[i]] == Rank {
					local_data[i] = keys[i]
					//log.Println(keys[i], "in local")
					//local_data = append(local_data, keys[i])
				} else {
					item := cache.Get(strconv.Itoa(keys[i]))
					if item == nil {
						pos, ok := partition[keys[i]]
						//log.Println(strconv.Itoa(keys[i]), "in remote")
						if ok {
							remote_data[pos] = append(remote_data[pos], keys[i])
							remote_data_dict[keys[i]] = i
						} else {
						...
						lru_result[i] = item.Value()
						//log.Println(keys[i], "in lru")
					}
		...
		if len(remote_data) == 0 {
			...
			for pos, keys := range remote_data {
				...
				for i, k := range keys {
					...
					cache.Set(strconv.Itoa(k), fdata[i], time.Minute*10)
					//log.Println("set cache", i, k)
				}
				err := json.NewEncoder(w).Encode(result)
				if err != nil {
					w.WriteHeader(http.StatusInternalServerError)
